...
export class InvoiceGenerator {
  static generateInvoiceHTML(invoice: any): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  static async generateInvoicePDF(invoice: any): Promise<string> {
...
router.post('/create', requireAuth, async (req, res) => {
  try {
    ...
    await AuditLogger.logPaymentCreation(userId, bookingId, payment.id, amount)
 ...
 */
router.post('/verify', requireAuth, requireRole(['OWNER']), async (req, res) => {
  try {
 ...
 */
router.get('/pending', requireAuth, requireRole(['OWNER']), async (req, res) => {
  try {
    const ownerId = req.currentUser!.id
 ...
 */
router.get('/:paymentId/audit', requireAuth, async (req, res) => {
  try {
