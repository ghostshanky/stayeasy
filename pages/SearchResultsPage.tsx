                        ...
                        <div className="space-y-6 bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800">
                            <div>
                                ...
                                <div className="relative h-1 w-full rounded-full bg-gray-200 dark:bg-gray-700">
                                    <div className="absolute h-1 rounded-full bg-primary" style={{ left: '10%', right: '40%' }}></div>
                                    <div className="absolute -top-1.5 h-4 w-4 rounded-full bg-primary border-2 border-white dark:border-gray-900 cursor-pointer" style={{ left: '10%' }}></div>
                                    <div className="absolute -top-1.5 h-4 w-4 rounded-full bg-primary border-2 border-white dark:border-gray-900 cursor-pointer" style={{ right: '40%' }}></div>
                                </div>
                ...
                <div className="lg:col-span-8 xl:col-span-9">
                    <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
                         <p className="text-[#111518] dark:text-white text-2xl sm:text-3xl font-black leading-tight tracking-[-0.033em] min-w-72">
                            {loading ? 'Searching for stays...' : `${listings.length > 0 ? `${listings.length}+` : 'No'} stays found`}
                        </p>
                    </div>
                    ...
                    {!loading && !error && (
                        <div className="grid grid-cols-1 @md:grid-cols-2 @4xl:grid-cols-3 gap-6">
                            {listings.map(listing => <ListingCard key={listing.id} listing={listing} navigate={navigate} />)}
                        </div>
                    )}
                </div>
            </div>
